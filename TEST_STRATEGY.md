...
7. **Restructure Project**: I would definitely restructure the project, create more helpers, add logging templates, and separate code execution from logging. Mainly create a scalable template that is easy to maintain.
...
- General set-up: Fixed dependency issues and integration misconfiguration.
- Comparing various approaches: Benefits vs disadvantages analysis.
...
- Scenario coverage: When debating scenario coverage, AI missed a lot of edge case scenarios.
- Contract logic understanding: AI failed to understand what would happen if someone did not reveal during the reveal period (as the funds were locked indefinitely). I had to redeploy a new contract with updated logic.
- Incorrect assertions: Generated incorrect assertions based on my requirements. The assertions had to be double-checked and enforced, as scenarios passed even with incorrect values.

**Overall**: In the end, 90%+ of the code here is generated by AI based on my requirements, and I was the one overseeing, checking, and reviewing all that was done. I am using Cursor and usually use TypeScript + ethers.js (with the contract ABI) to interact with my needs, along with many RPC "read" methods from the chain to validate assertions (based on parsed events in ~90% of cases).

My recent projects involve testing a solver that performs swaps. I found a cool set-up in Golang with W3VM which I had no idea how it worked and did not know Go, but I managed with AI to create comprehensive tests on Ethereum fork and Base fork, where I query Dune for CowSwap's last 30 days' top 50 pairs sorted by USD value, then create them into a JSON file with scenarios and feed it to this Go set-up to execute scenarios on a forked state of Ethereum or Base chain.

I would not be great at live coding, but I can handle whatever task I have in front of me with the help of AI. 
